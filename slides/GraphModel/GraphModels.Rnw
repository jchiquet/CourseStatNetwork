...
\begin{frame}
  ...
  \begin{align*}
    \mathbf{Z}_i = \left\{\mathbf{1}_{\{i \in \bullet\}}\right\}_{\bullet\in\mathcal{Q}} = (\mathbf{1}_{\{i \in \textcolor{yellow!40!orange}{\bullet}\}}, \mathbf{1}_{\{i \in \textcolor{blue!80!black}{\bullet}\}}, \mathbf{1}_{\{i \in \textcolor{green!50!black}{\bullet}\}}) = \ & \sim^{\text{iid}} \mathcal{M}(1,\alpha), \\
...
\begin{frame}
  ...
  \begin{block}{Family of distribution that factorizes}
      ...
      \begin{equation*}
        \mathbb{Q}(\bZ) = \prod_{i=1}^n \mathbb{Q}_i(\mathrm{cl}(i)) = \prod_{i=1}^n\prod_{q=1}^Q \tau_{iq}^{Z_{iq}},
      \end{equation*}
      where $\tau_{iq} =\mathbb{Q}_i(i \in q) = \E_{\mathbb{Q}}(Z_{iq})$, with $\sum_{q} \tau_{iq} = 1$ for all $i=1,\dots,n$.
  \end{block}
